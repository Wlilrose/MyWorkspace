...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.webServer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.web-servers.update", [$webServer->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="server_name">{{ trans('cruds.webServer.fields.server_name') }}</label>
                <input class="form-control {{ $errors->has('server_name') ? 'is-invalid' : '' }}" type="text" name="server_name" id="server_name" value="{{ old('server_name', $webServer->server_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip_address') ? 'is-invalid' : '' }}" type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $webServer->ip_address) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('operating_system') ? 'is-invalid' : '' }}" type="text" name="operating_system" id="operating_system" value="{{ old('operating_system', $webServer->operating_system) }}">
